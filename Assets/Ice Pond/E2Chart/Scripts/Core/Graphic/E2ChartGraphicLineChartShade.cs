...
namespace E2C.ChartGraphic
{
    public class E2ChartGraphicLineChartShade : E2ChartGraphic
    {
            ...
            if (curve)
            {
                if (inverted)
                {
                    ...
                    for (int i = m_start + 1; i <= m_end; ++i)
                    {
                        ...
                        CreateBerzierCurve(p0, p1, t0, t1, ps0, ps1, ts0, ts1, points, colors[colorIndex]);
                    }
                }
                else
                {
        ...
        protected void CreateBerzierCurve(Vector2 p0, Vector2 p1, Vector2 t0, Vector2 t1, Vector2 ps0, Vector2 ps1, Vector2 ts0, Vector2 ts1, Vector2[] points, Color c)
        {
            ...
            for (int j = 1; j <= stepCount; ++j)
            {
                ...
                AddPolygonRectIntersect(points, c);
            }
        }
    }
}