...
namespace E2C.ChartBuilder
{
    public class E2ChartDataInfoLinear : E2ChartDataInfo
    {
        ...
        public void ComputeRange(bool fullRange, float rMin, float rMax)
        {
            ...
            if (fullRange)
            {
                ...
                if (Mathf.Approximately(minPos, float.MaxValue)) { minPos = 0.0f; maxPos = 1.0f; }
            }
            else
            {
                minPos = rMin;
            ...
            if (minPos >= maxPos) maxPos = minPos + 1;
