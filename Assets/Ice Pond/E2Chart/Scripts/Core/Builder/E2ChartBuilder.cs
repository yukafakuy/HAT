...
namespace E2C.ChartBuilder
{
    public abstract class E2ChartBuilder
    {
        ...
        public E2ChartCustomLabels customLabels;
        ...
        public bool isLinear { get => (chartType == E2Chart.ChartType.LineChart || chartType == E2Chart.ChartType.AreaChart) && options.xAxis.type != E2ChartOptions.AxisType.Category; }
        public bool isDateTime { get => (chartType == E2Chart.ChartType.LineChart || chartType == E2Chart.ChartType.AreaChart) && options.xAxis.type == E2ChartOptions.AxisType.DateTime; }
        ...
        {
            return seriesColors[index % seriesColors.Count];
        }
        ...
        public void Init()
        {
            ...
            if (!VerifyChartRect()) { Debug.Log("Invalid chart rectangle"); return; }
...
#else
                try { options.plotOptions.generalFont = Resources.GetBuiltinResource<Font>("Arial.ttf"); }
                catch { options.plotOptions.generalFont = Resources.GetBuiltinResource<Font>("LegacyRuntime.ttf"); }
#endif
            }

            if (chartType != E2Chart.ChartType.AreaChart) labelContent = string.IsNullOrEmpty(options.label.content) ? "{dataY}" : options.label.content;
            ...
            customLabels = chart.GetComponent<E2ChartCustomLabels>();

            //colors
            if (options.plotOptions.seriesColors == null || options.plotOptions.seriesColors.Length == 0)
            {
                seriesColors = new List<Color>(); seriesColors.Add(Color.white);
            }
            else
            {
                seriesColors = new List<Color>(options.plotOptions.seriesColors);
            }

            if (options.plotOptions.colorMode == E2ChartOptions.ColorMode.ByData)
            {
                List<int> colorInterval = new List<int>();
                if (options.plotOptions.colorInterval == null || options.plotOptions.colorInterval.Length == 0)
                {
                    colorInterval.Add(1);
                }
                else
                {
                    for (int i = 0; i < options.plotOptions.colorInterval.Length; ++i)
                    {
                        int interval = options.plotOptions.colorInterval[i] > 0 ? options.plotOptions.colorInterval[i] : 1;
                        colorInterval.Add(interval);
                    }
                }
                ...
                {
                    if (i < colorInterval.Count)
                    {
                        for (int j = 0; j < colorInterval[i]; ++j)
                        {
                            colorList.Add(seriesColors[i]);
                        }
                    }
                    else
                    {
        ...
        public void Build()
        {
            ...
            if (options.legend.enable && legendRect == null && chartType != E2Chart.ChartType.Gauge) CreateLegends();
            ...
            CreateBackground();
            CreateItems();
            if (options.label.enable || chartType == E2Chart.ChartType.Table) CreateLabels();
            if (customLabels != null) CreateCustomLabels();
        ...
        protected virtual void CreateCustomLabels() { }
