    ...
    public class RadialAxis : E2ChartGridAxis
    {
        ...
        public override void CreateLabels()
        {
            ...
            for (int i = 0; i < labelCount; ++i)
            {
                ...
                {
                    if (Mathf.Approximately(pos.x, 0.0f) || Mathf.Approximately(pos.x, 180.0f))
                    {
                        label.alignment = E2ChartBuilderUtility.ConvertAlignment(TextAnchor.MiddleCenter);
                        float widthLimit = labelRect.rect.width;
                        E2ChartBuilderUtility.TruncateText(label, widthLimit);
                    }
                    else
                    {
                        label.alignment = pos.x > 0.0f ?
                            E2ChartBuilderUtility.ConvertAlignment(TextAnchor.MiddleLeft) :
                            E2ChartBuilderUtility.ConvertAlignment(TextAnchor.MiddleRight);
