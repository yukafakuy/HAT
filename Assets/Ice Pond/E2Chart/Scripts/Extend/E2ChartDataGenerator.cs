...
namespace E2C
{
    public class E2ChartDataGenerator : MonoBehaviour
    {
        ...
        public struct SeriesInfo
        {
            ...
            [NonReorderable] public float[] dataYMultiplier;
            public float dataZMin;
            public float dataZMax;
            [NonReorderable] public float[] dataZMultiplier;
            public string minDateTimeString;
            public string maxDateTimeString;
            [NonReorderable] public string[] dataName;
#else
            public int dataCount;
            public float dataXMin;
            public float dataXMax;
        ...
        public void GenerateData()
        {
            ...
            for (int i = 0; i < seriesCount; ++i)
            {
                ...
                if (dataZ)
                {
                    float[] dataZMultipliers = info.dataZMultiplier == null || info.dataZMultiplier.Length == 0 ? new float[] { 1.0f } : info.dataZMultiplier;
                    series.dataZ = new List<float>();
                    for (int j = 0; j < info.dataCount; ++j)
                    {
                        float dZ = Random.Range(info.dataZMin, info.dataZMax);
                        dZ *= GetMultiplier(j / (info.dataCount - 1.0f), dataZMultipliers);
                        series.dataZ.Add(dZ);
                    }
                }
