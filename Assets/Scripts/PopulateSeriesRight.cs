...
public class PopulateSeriesRight : MonoBehaviour
{
    //data
    ...
    private string ConvertToDashedDate(string input)
    {
        try
        {
            DateTime parsedDate = DateTime.ParseExact(input, "yyyy-MM-dd", null);
            return parsedDate.ToString("yyyy-MM-dd");
        }
        catch (FormatException)
        {
            Debug.LogError("Invalid date format. Expected yyyy-MM-dd");
            return string.Empty;
        }
    }
